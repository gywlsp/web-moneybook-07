const getRouter = () => {
  ...
  Router.prototype.init = (key, defaultValue) => {
    ...
    window.history.pushState({}, null, defaultValue);
    return key;
  };
  ...
  Router.prototype.set = (key, newValue, pushState = true) => {
    if (!(key in state)) throw Error('존재하지 않는 key값 입니다.');
    state[key].state = newValue;
    if (pushState) window.history.pushState({}, null, newValue);
    Router.prototype.notify(key);
  };
  ...
  window.addEventListener('popstate', () =>
    router.set('pathname', getPathname(), false));
